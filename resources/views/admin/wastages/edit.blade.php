...
<main class="app-content">
  <form class="form-horizontal" method="POST" action="{{ route('wastages.update', $wastage->id) }}"> 
  @csrf
  @method('PUT')
  ...
  <div class="app-title">
    <div>
      ...
      <p>Update Wastage Form</p>
    </div>
    <ul class="app-breadcrumb breadcrumb">
      <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
      <li class="breadcrumb-item">Wastages</li>
      <li class="breadcrumb-item"><a href="#">Edit Wastage</a></li>
    </ul>
  </div>
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="tile">
        ...
        <h3 class="tile-title">Edit New Wastage</h3>
        ...
        <div class="tile-body">
          ...
          <div class="form-group row add_asterisk">
            <label class="control-label col-md-3">Wastage Name</label>
            <div class="col-md-8">
              <input name="name" value="{{ old('name') ?: $wastage->name }}" class="form-control" type="text" placeholder="Wastage Name">
          ...
          <div class="form-group row">
            ...
            <div class="col-md-8">
              <textarea name="description" id="" cols="30" rows="10" class="form-control" placeholder="Wastage Description">{{ old('description') ?: $wastage->description }}</textarea>
          ...
          <div class="row">
            <div class="col-md-12">
              <button class="btn btn-primary pull-right" type="submit" type="submit"><i
                  class="fa fa-fw fa-lg fa-check-circle"></i>Edit Wastage</button>
            </div>
          </div>
        </div>
...
var i = 1;
  function addRow(tableId){
    ...
    var tr = '<tr>' +
            '<td><select name=\"wastage_ids[]\" id=\"wastage_name_'+i+'\" class=\"dynamic_wastage form-control select2\" onchange=\"show_wastage_price(this.value, '+i+')\"><option>select</option></select>' +
            '</td>'+
            '<td><input type=\"number\" min=\"1\" name=\"unit_prices[]\" id=\"wastage_price_'+i+'\" placeholder=\"\" class=\"dynamic_wastage_price form-control\"></td>'+
            ...
            '<td><input type=\"number\" readonly=\"readonly\" placeholder=\"Sub Total\" name=\"wastage_sub_total[]\" id=\"sub_total_'+i+'\" class=\"form-control\" ></td>'+
            '<td><button class=\"remove btn-danger\"><span class=\"fa fa-trash-o\"></span></button></td>'+
            '</tr>';

            $('#wastage_table tbody').append(tr);



  $.ajax({
      url: "{{ url('wastages/get_json_list') }}",
      type: "GET",
      success:function(res){
        if(res){
          $.each(res, function(id, wastage_name){
            $("#wastage_name_"+i).append('<option value="'+id+'">'+wastage_name+'</option>');
          });
        }
      }
    });
  ...
  function show_wastage_price(wastage_id, n){
    var wastage_id = wastage_id;
    $.ajax({
      url: "{{ url('wastages/get_wastage_by_id') }}?id="+wastage_id,
      type: "GET",
      success:function(res){
        if(res){
          $.each(res, function(key, value){
            $('#wastage_price_'+n).val(value['wastage_price']);
              $('#wastage_unit_'+n).html(value['wastage_unit']['name']);
          });
        }
      }
    });
  ...
  function get_sub_total(qty, n){
    $("#wastage_price_"+n).prop('readonly', true);
    var qty = parseInt(qty);
    var wastage_price = parseFloat($('#wastage_price_'+n).val());
      $("#sub_total_"+n).val(qty*wastage_price);
      var inv_sub_total = 0;
      for(var i = 1; i<=n; i++){
