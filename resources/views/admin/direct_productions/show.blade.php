      ...
      <div class="tile">
        <div class="row display_when_print" style="display: none;">
          <div class="col-md-12 text-center">
            <h4>{{ $directProduction->company->name }}</h4>
            <h5>{{ $directProduction->company->address }}</h5>
            <h6>{{ $directProduction->company->phone }}</h6>
            <h6>{{ $directProduction->company->email }}</h6>
            <br>
          </div>
        </div>
    <br>
        ...
        <div class="row">
          <div class="col-md-12">
            <table class="table no-spacing">
              <tbody>
                <tr>
                  <td>Production ID : {{ $directProduction->id }}</td>
                  <td>Date : {{ $directProduction->direct_production_date }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            @php
                $qty_total_kg = 0;
            @endphp
            @foreach ($directProduction->direct_production_sheets as $sheet)
            @php
                $qty_kg = $sheet->qty_kg;
                $qty_total_kg += $qty_kg;
            @endphp
            <tr>
              <td style="padding-top:2px; padding-bottom:2px">{{ $sheet->direct_production_sheetable->raw_material->name.' - '.$sheet->direct_production_sheetable->name }}</td>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            ...
            @endphp
            <tr>
              <td style="padding-top:2px; padding-bottom:2px">{{ $details->direct_production->direct_production_sheets->first()->direct_production_sheetable->raw_material->name.'-'.
                  $details->direct_production->direct_production_sheets->first()->direct_production_sheetable->name }}</td>
              <td style="padding-top:2px; padding-bottom:2px">{{ $details->product->raw_material->name.' - '.$details->product->name }}</td>
              <td style="padding-top:2px; padding-bottom:2px">{{ $details->machine ? $details->machine->name:($details->product->machine ? $details->product->machine->name:'-') }}</td>
              ...
              <td style="padding-top:2px; padding-bottom:2px">{{ $pack }}</td>
              <td style="padding-top:2px; padding-bottom:2px">{{ $net_weight.' '.config('app.kg') }}</td>
              <td style="padding-top:2px; padding-bottom:2px">{{ $details->fm_kutcha->raw_material->name.' - '.$details->fm_kutcha->name }}</td>
