                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            @foreach ($receivable_amounts as $key => $this_month_receivable)
                                @php
                                $this_month_amount = $this_month_receivable->thisMonthReceivable($this_month_receivable->party_id);
                                $total_this_month += $this_month_amount;

                                $previous_month_amount = $this_month_receivable->previousMonthReceivable($this_month_receivable->party_id);
                                $total_previous_month += $previous_month_amount;
                                @endphp
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ $this_month_receivable->party->name }}</td>
                                    <td>{{ $this_month_amount }}
                                    </td>
                                    <td>{{ $previous_month_amount }}
                                    </td>
                                </tr>
                            @endforeach
                            <tr>
