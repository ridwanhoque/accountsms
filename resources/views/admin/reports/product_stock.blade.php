                ...
                <tbody>
                  {{-- @dd($product_stocks->first()) --}}
                    @isset($product_stocks)
                        ...
                        <tr>
                          <td>{{ $product_stock->product->raw_material->name.' - '.$product_stock->product->name }}</td>
	            						<td>{{ $product_stock->opening_quantity }} / {{ $product_stock->opening_pack }} / {{ $product_stock->opening_weight }}</td>
                          <!-- <td>{{ $product_stock->product->produced_kg()->isEmpty() ? 0.00:$product_stock->product->produced_kg()[0] }} / 
						              {{ $product_stock->product->produced_kg_direct()->isEmpty() ? 0.00:$product_stock->product->produced_kg_direct()[0] }}
						              </td> -->
                          <td>{{ $product_stock['produced_quantity'] ?: 0.00 }}</td>
                          <td>{{ $product_stock['produced_pack'] }}</td>
                          <td>{{ $product_stock['produced_weight'] }}</td>
                          
                          <td>{{ $product_stock['sold_quantity'] }}</td>
                          <td>{{ $product_stock['sold_pack'] ?: 0 }}</td>
                          <td>{{ $product_stock['sold_weight'] }}</td>
