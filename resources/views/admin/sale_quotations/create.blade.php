...
<main class="app-content">
  <form class="form-horizontal" method="POST" action="{{ route('sale_quotations.store') }}">
    @csrf

    <div class="app-title">
      <div>
        <h1><i class="fa fa-edit"></i> Sale Quotation</h1>
        <p>Create Sale Quotation Form</p>
      </div>
      <ul class="app-breadcrumb breadcrumb">
        <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
        <li class="breadcrumb-item">Sale Quotations</li>
        <li class="breadcrumb-item"><a href="#">Add Sale Quotation</a></li>
      </ul>
    </div>
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="tile">
          ...
          <h3 class="tile-title">Add New Sale Quotation</h3>
            ...
            <div class="form-group row add_asterisk">
              ...
              <div class="col-md-8">
                <input name="sale_quotation_date"
                  value="{{ old('sale_quotation_date') == '' ? date('Y-m-d'):old('sale_quotation_date') }}" class="form-control dateField"
                ...
                <div class="col-md-12">
                  <table class="table no-spacing" id="tab_logic">
                    ...
                    <tbody>
                      ...
                      @foreach (old('product_ids') as $old_key => $old_product_id)
                      <tr id='addr0'>
                          <td>1</td>
                          <td>
                            <select name="product_ids[]" class="form-control raw_material small_input_box">
                              <option value="0">select</option>
                              @foreach ($products as $product)
                              <option value="{{ $product->id }}" {{ $product->id == old('product_ids')[$old_key] }}>{{ $product->name }}</option>
                              @endforeach
                            </select>
                            <div class="text-danger">
                              {{ $errors->has('product_ids')[$old_key] ? $errors->first('product_ids')[$old_key]:'' }}
                            </div>
                          </td>
                  ...
                  <table class="table" id="tab_logic_total">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="text-center"><input type="number" name='invoice_vat' value="0"
              ...
              <div class="row">
                <div class="col-md-12">
                  <button class="btn btn-primary pull-right" type="submit" type="submit"><i
                      class="fa fa-fw fa-lg fa-check-circle"></i>Add Sale Quotation</button>
                </div>
              </div>
            </div>
...
function calc_total()
{
  ...
  var discount = $('#invoice_discount').val();
	$('#total_amount').val(((invoice_vat+tax_sum+total)-discount).toFixed(3));
  $('#total_paid').val(((invoice_vat+tax_sum+total)-discount).toFixed(3));
}
</script>
