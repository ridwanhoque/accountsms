...
<main class="app-content">
  <form class="form-horizontal" method="POST" action="{{ route('sheet_productions.store') }}" id="sheet_production_form">
    @csrf
    <input type="hidden" name="temporary_sheet_production_id" value="{{ $id }}">
    <div class="app-title">
      <div>
        ...
        <div class="tile">
          ...
          <div class="tile-body">
            ...
            <div class="container">
                      ...
                      <tr data-repeater-item class="outer" valign="top" id="addr_raw_material_table0">
                        ...
                        <td>
                          <select name="batch_id[]" onchange="material_current_stock(this)" class="form-control small_input_box batches">
                            <option value="0">select</option>
                            <option value="" {{ $temp_material->batch_id == null ? 'selected':'' }}>-- opening --</option>
              ...
              <div class="mt-2"></div>
              @if($temp_assign_kutchas->isNotEmpty())
              <div class="row clearfix">
                <div class="col-md-12">
              ...
              @if(!$temp_kutcha_wastages->isEmpty())
              <div class="row clearfix">
                <div class="col-md-12">
              ...
              <input type="hidden" name="sheet_with_wastage" id="sheet_with_wastage" value="">
...
<script>
  $('.materials').each(function() {
    ...
    $.ajax({
      ...
      type: 'GET',
      data: 'id=' + raw_material_id + '&batch_id=' + batch_id,
  ...
  $('.fm_kutcha_in_ids').each(function() {
    ...
    $.ajax({
      ...
      success: function(res) {
        row.find('.fm_kutcha_in_stock').val(res);
      }
    });
  ...
  function show_total_input() {
    ...
    $('#total_input').val(total_input.toFixed(2));
  }
  ...
  function show_total_output() {
    ...
    $('#total_output').val(total_output.toFixed(2));
  }
  ...
  $(document).ready(function() {
    ...
    function total_kgs_details() {
      ...
      $('#total_kg_details').val(total_kgs_details.toFixed(2));
    }
    ...
    function show_sheet_with_wastage() {

      var total_sheet_kgs = $('#total_kg_details').val() > 0 ? $('#total_kg_details').val() : 0;
      var total_wastage_kgs = $('#total_output').val() > 0 ? $('#total_output').val() : 0;
      $('#sheet_with_wastage').val((parseFloat(total_sheet_kgs) + parseFloat(total_wastage_kgs)).toFixed(2));
    }

    show_total_input();
    show_total_output();
    show_sheet_with_wastage();
